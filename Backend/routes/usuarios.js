...
router.put("/change-password", async (req, res) => {
  ...
  if (!username || !oldPassword || !newPassword) return res.status(400).json({ error: "Faltan datos" });
  ...
  db.prepare("UPDATE users SET password = ? WHERE username = ?").run(hashedNew, username);
...
router.post("/delete", (req, res) => {
  ...
  if (info.changes === 0) return res.status(400).json({ error: "Usuario no encontrado" });
